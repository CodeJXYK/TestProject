...
@interface BaseNC ()<UIGestureRecognizerDelegate,UINavigationControllerDelegate>

@property (nonatomic , assign) id systemPanTarget; //系统侧滑返回代理对象
@property (nonatomic , assign) SEL systemPanHandler; //系统侧滑返回回调

@property (nonatomic , strong) BaseVC *curVC; //用于记录侧滑返回的当前视图控制器
@property (nonatomic , strong) BaseVC *preVC; //用于记录侧滑返回的前一个视图控制器
...
- (void)backPanHandler:(UIPanGestureRecognizer *)pan{
    if(_systemPanTarget && _systemPanHandler){
        ...
        if(pan.state == UIGestureRecognizerStateBegan){
            ...
            _curVC = [tmpCurVC isKindOfClass:[BaseVC class]] ? (BaseVC *)tmpCurVC : nil;
            _preVC = [tmpPreVC isKindOfClass:[BaseVC class]] ? (BaseVC *)tmpPreVC : nil;
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer {
    ...
    if([lastVC isKindOfClass:[BaseVC class]]){
        BaseVC *vc = (BaseVC *)lastVC;
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    UIViewController *lastVC = self.viewControllers.lastObject;
    if([lastVC isKindOfClass:[BaseVC class]]){
        BaseVC *lvc = (BaseVC *)lastVC;
        self.alpha = lvc.navigationBarAlpha;
        if(viewController && [viewController isKindOfClass:[BaseVC class]]){
